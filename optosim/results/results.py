...
def parse_args():
    ...
    parser.add_argument(
        "nmax",
        type=int,
        help="The number of events to plot",
        default=20_000
    )
...
class Results:
    def __init__(self, test_run_id, train_run_id, nmax, normalise=False, stupid_pmts=2, great_pmts=5, finest_pmts=20, save_figures=False):
    ...
    def get_wa_model(self):
        ...
        wa_pred = weighted_average_estimator(self.X_test, r=self.cylinder_radius)
    ...
    def _get_models(self):
        ...
        models_to_read = [os.path.basename(x) for x in glob.glob(os.path.join(MODEL_DIR, self.train_run_id, f"*{self.train_run_id}*.pkl"))]
    ...
    def _define_comparison_models(self):
        ...
        self.stupid_model_name = get_filename_from_pmts(
            self._stupid_pmts, 
            self.train_run_id, 
            self.suffix
        )

        self.great_model_name = get_filename_from_pmts(
            self._great_pmts, 
            self.train_run_id, 
            self.suffix
        )

        self.finest_model_name = get_filename_from_pmts(
            self._finest_pmts, 
            self.train_run_id, 
            self.suffix
        )
    ...
    def do_plot_1d_histogram(self, which='dist'):
        ...
        if which == 'dr':
            ...
            labels = ["Weighted Avg.", f"{self._stupid_pmts} PMTs", f"{self._great_pmts} PMTs", f"{self._finest_pmts} PMTs"]
    ...
    def do_plot_statistics(self, which='dr', nbins=30):
        ...
        if which == 'r':
            ...
            statistics = compute_statistics(self.r, self.pos_test, [self.wa_pred, self.stupid_pred_pos, self.great_pred_pos, self.finest_pred_pos], bins, func_xy_dist)
            ...
            statistics = compute_statistics(self.n_detected_photon, self.pos_test, [self.wa_pred, self.stupid_pred_pos, self.great_pred_pos, self.finest_pred_pos], bins, func_xy_dist)
    ...
    def do_plot_2d_hist_with_weights(self):
        ...
        for model in [self.stupid_model_name, self.great_model_name, self.finest_model_name]:
            _pmts = self.results[model]['PMTs']
            fig, ax = plot_2d_histogram_with_weights(self.pos_test, self.results[model]['pred'], model, f"{_pmts}x{_pmts} PMTs")
...
def plot_pmt_and_tpc(ax, radius, cylinder_radius):
